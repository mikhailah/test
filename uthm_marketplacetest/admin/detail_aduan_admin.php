...
// Get aduan details
$sql = "SELECT a.*, p.nama_penuh, p.emel, p.no_telefon FROM aduan a 
        LEFT JOIN pengguna p ON a.id_pengguna = p.id_pengguna 
        WHERE a.id_aduan = '$id_aduan'";
$result = mysqli_query($conn, $sql);
$aduan = mysqli_fetch_assoc($result);

if(!$aduan) {
    die("Aduan tidak ditemui.");
}
...
// Get chat messages
$sql_chat = "SELECT ac.*, p.nama_penuh FROM aduan_chat ac 
             LEFT JOIN pengguna p ON ac.id_pengguna = p.id_pengguna 
             WHERE ac.id_aduan = '$id_aduan' 
             ORDER BY ac.tarikh_hantar ASC";
...
$status_options = ['baru', 'dalam_proses', 'selesai', 'ditolak'];
?>

<!DOCTYPE html>
<html>
<head>
    <title>Detail Aduan | Admin</title>
    ...
    <style>
        ...
        .meta { color: #888; font-size: 0.85rem; margin: 0.5rem 0 1rem 0; }
        .meta span { margin-right: 1rem; }
        
        .user-info { background: #f8f9fc; padding: 1rem; margin: 1rem 0; border-left: 4px solid #6c3cff; }
        .user-info strong { display: block; margin-bottom: 0.3rem; }
        ...
        .description { background: #f8f9fc; padding: 1rem; margin: 1rem 0; }
        ...
        .lampiran-item .not-found { font-size: 0.7rem; color: #dc2626; padding: 0.5rem; }
        
        .tindakan-form { background: #f8f9fc; padding: 1.5rem; margin: 1rem 0; border: 1px solid #eef2f8; }
        .tindakan-form label { display: block; font-weight: 600; margin-bottom: 0.3rem; }
        .tindakan-form select, .tindakan-form textarea { width: 100%; padding: 0.8rem; border: 1px solid #ddd; border-radius: 0px; margin-bottom: 0.8rem; }
        .tindakan-form button { background: #6c3cff; color: white; border: none; padding: 0.8rem 2rem; font-weight: 600; cursor: pointer; }
        .tindakan-form button:hover { opacity: 0.9; }
        
        .tindakan-done { background: #d1fae5; padding: 1rem; margin: 1rem 0; border-left: 4px solid #10b981; }
        .tindakan-done strong { display: block; margin-bottom: 0.3rem; }
        
        .chat-box { margin-top: 2rem; border-top: 1px solid #eef2f8; padding-top: 1.5rem; }
        .chat { margin-bottom: 1rem; }
        .chat .msg { padding: 0.5rem 1rem; border-radius: 0px; max-width: 80%; display: inline-block; }
        .chat.user .msg { background: #6c3cff; color: white; text-align: left; }
        .chat.admin .msg { background: #f1f3f5; color: #1a1a2e; text-align: left; }
        ...
        .btn-back-primary { background: #6c3cff; color: white; border: none; padding: 0.8rem 2rem; font-weight: 600; cursor: pointer; text-decoration: none; display: inline-block; }
        .btn-back-primary:hover { opacity: 0.9; }
    ...
    <div class="main-content">
        <div class="container">
            <div style="margin-bottom:1rem;display:flex;gap:1rem;flex-wrap:wrap;">
                <a href="admin_aduan.php" class="btn-back">← Kembali ke Senarai</a>
                <a href="../aduan/detail_aduan.php?id=<?= $id_aduan; ?>" class="btn-back-primary">Lihat sebagai Pengguna</a>
            </div>
            ...
            <div class="detail-card">
                <div class="header-top">
                    <div>
                        ...
                        <div class="meta">
                            <span>#ADU-<?= str_pad($aduan['id_aduan'], 5, '0', STR_PAD_LEFT); ?></span>
                ...
                <div class="user-info">
                    ...
                    <?= htmlspecialchars($aduan['nama_penuh'] ?? 'Unknown'); ?><br>
                    <?= htmlspecialchars($aduan['emel'] ?? 'Tiada'); ?><br>
                    <?= htmlspecialchars($aduan['no_telefon'] ?? 'Tiada'); ?>
                </div>
                    ...
                    <div class="lampiran-section">
                        ...
                        <div style="display:flex;flex-wrap:wrap;gap:0.5rem;margin-top:0.5rem;">
                            <?php while($lampiran = mysqli_fetch_assoc($result_lampiran)): 
                                $lampiran_path = "../aduan/lampiran/" . $lampiran['nama_file'];
                            ...
                            ?>
                                <div class="lampiran-item">
                                    ...
                                    <?php else: ?>
                                        <div class="not-found">Fail tidak ditemui</div>
                                    <?php endif; ?>
                ...
                <?php if(!empty($aduan['tindakan'])): ?>
                    <div class="tindakan-done">
                        <strong>Tindakan Sedia Ada</strong>
                        <?= nl2br(htmlspecialchars($aduan['tindakan'])); ?>
                        <br><small style="color:#888;"><?= date('d/m/Y H:i', strtotime($aduan['tarikh_tindakan'])); ?></small>
                    </div>
                <?php endif; ?>
                ...
                <?php if($aduan['status'] != 'selesai' && $aduan['status'] != 'ditolak'): ?>
                    <div class="tindakan-form">
                        ...
                        <form action="proses_tindakan_admin.php" method="POST">
                            <input type="hidden" name="id_aduan" value="<?= $id_aduan; ?>">
                            
                            <label>Update Status</label>
                            <select name="status">
                                <?php foreach($status_options as $opt): ?>
                                    <option value="<?= $opt; ?>" <?= $aduan['status']==$opt?'selected':''; ?>><?= ucfirst(str_replace('_', ' ', $opt)); ?></option>
                                <?php endforeach; ?>
                            </select>
                            
                            <label>Tindakan / Nota</label>
                            <textarea name="tindakan" rows="4" placeholder="Terangkan tindakan yang diambil..."><?= $aduan['tindakan']; ?></textarea>
                ...
                <div class="chat-box">
                    ...
                    <?php if(mysqli_num_rows($result_chat) > 0): ?>
                        <?php while($chat = mysqli_fetch_assoc($result_chat)): ?>
                            <div class="chat <?= ($chat['id_pengguna'] == $id_pengguna) ? 'admin' : 'user'; ?>">
                                <div>
                                    <div class="sender"><?= ($chat['id_pengguna'] == $id_pengguna) ? 'Admin' : htmlspecialchars($chat['nama_penuh'] ?? 'Pengguna'); ?></div>
                        ...
                        <?php endwhile; ?>
                    <?php else: ?>
                        <p style="color:#888;">Tiada perbualan.</p>
                    <?php endif; ?>

                    <form class="chat-input" action="../aduan/hantar_mesej_aduan.php" method="POST">
                        <input type="hidden" name="id_aduan" value="<?= $id_aduan; ?>">
                        <input type="text" name="mesej" placeholder="Hantar mesej kepada pengguna..." required autocomplete="off">
